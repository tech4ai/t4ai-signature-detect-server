...
def initialize_predictor(choice):
    ...
    try:
        if choice == "Requests HTTP":
            current_predictor = HttpPredictor(url="REDACTED_TRITON_URL")
        elif choice == "Triton Client":
            current_predictor = TritonClientPredictor(url="REDACTED_TRITON_URL")
        elif choice == "Vertex Endpoint":
            current_predictor = VertexAIPredictor(url="REDACTED_VERTEX_ENDPOINT")
        else:
