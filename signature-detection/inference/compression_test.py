...
import numpy as np
from predictors import TritonClientPredictor
...
class ConcurrentTritonClientPredictor(TritonClientPredictor):
    ...
    def request_with_compression(self, inputs: np.ndarray, compression_config: CompressionConfig) -> Tuple[np.ndarray, float]:
        infer_inputs = []
        x = inputs
        if x.dtype != self.np_input_formats[0]:
            x = x.astype(self.np_input_formats[0])
        infer_input = self.InferInput(self.input_names[0], [*x.shape], self.input_formats[0].replace("TYPE_", ""))
...
class CompressionTestPipeline:
    def __init__(self, predictor_url: str):
        ...
        self.compression_configs = [
            ...
            CompressionConfig(CompressionAlgorithm.DEFLATE, CompressionAlgorithm.DEFLATE),
        ]
        self.setup_logging()
    ...
    def make_prediction_with_retry(self, predictor: ConcurrentTritonClientPredictor, 
                                 image_data: np.ndarray, 
                                 config: CompressionConfig) -> Tuple[np.ndarray, float]:
        try:
    ...
    def test_config(self, image_paths: List[str], config: CompressionConfig) -> Dict:
        ...
        self.logger.info(f"Thread {thread_id} initialized with new predictor for config {config}")
        ...
        for image_path in image_paths:
            try:
                ...
                _, latency = self.make_prediction_with_retry(predictor, image_data, config)
    ...
    def run_concurrent_tests(self, image_paths: List[str]) -> List[Dict]:
        ...
        print(f"Starting concurrent compression tests with {len(self.compression_configs)} configurations")
        print(f"Total images to process per configuration: {len(image_paths)}\n")
        
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=len(self.compression_configs),
            thread_name_prefix="CompTest"
        ) as executor:
            futures = [
